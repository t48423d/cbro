...
@Slf4j
public class ExcelProductionPlanUtils {
    ...
    private static JSONArray readSheet(Sheet sheet) {
        ...
        Integer toYear = currentDate.getYear();
        for (int j = cellStart; j < cellEnd; j++) {
            ...
            if (val != null && val.trim().length() != 0) {
                if(!val.equals("Model")) {
                    val = ExcelDoubleToDate(val);
                }
                keyMap.put(j, val.replace(",", ""));
            }
        }
...
//        log.info("表头数据：" + JSON.toJSONString(keyMap));
...
//                log.info("第" + (i + 1) + "行，第" + (k + 1) + "列");
    ...
    private static String getCellValue(Cell cell) {
        ...
        if (cell.getCellTypeEnum() == CellType.NUMERIC) {
            ...
            if (dd == Math.floor(dd)) {
                //是整数
                return new BigDecimal(dd).toPlainString();
            } else {
                // 不是整数
                return new BigDecimal(dd).setScale(7,BigDecimal.ROUND_HALF_UP).toString();
            }
        }
        // 布尔值类型
    ...
    public static <T> void exportTemplate(HttpServletResponse response, String fileName, String sheetName,
        ...
        for (int i = 0; i < headFieldList.size(); i++) {
            ...
            headList.add(each.getName().replace(",", ""));
    ...
    private static int setCellValue(Cell cell, Object o, CellStyle style) {
        ...
        if (o instanceof String) {
            String s = o.toString();
            if (isNumeric(s) && s!=null && s.length() > 0) {
